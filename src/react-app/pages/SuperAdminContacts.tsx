...
export default function SuperAdminContacts() {
  ...
  useEffect(() => {
    if (searchTerm) {
      const filtered = contacts.filter(contact => 
        ...
        contact.admin_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        contact.admin_phone.toLowerCase().includes(searchTerm.toLowerCase())
      );
  ...
  const fetchContacts = async () => {
    try {
      const response = await apiCall('/api/superadmin/contacts');
      if (response.ok) {
  ...
  const downloadContacts = () => {
    ...
    const exportData = filteredContacts.map(contact => ({
      'Organización': contact.organization_name,
      'Nombre del Administrador': contact.admin_name,
      'Teléfono / WhatsApp': contact.admin_phone
    }));
    ...
    XLSX.writeFile(workbook, `contactos_administradores_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleDelete = (contact: Contact) => {
    setConfirmModal({
      ...
      message: `¿Estás seguro de que quieres eliminar el contacto de "${contact.admin_name}"? Esta acción actualizará el usuario administrador.`,
      onConfirm: async () => {
        try {
          const response = await apiCall(`/api/superadmin/contacts/${contact.user_id}`, {
            method: 'DELETE',
          });
      ...
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-gray-600">Lista de contactos únicos de administradores</p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center space-x-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Buscar por organización, nombre o teléfono..."
                value={searchTerm}
                ...
                {paginatedContacts.map((contact, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{contact.organization_name}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{contact.admin_name}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{contact.admin_phone}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {searchTerm 
                ...
                : 'No hay contactos de administradores registrados.'
              }
            </p>
          </div>
        )}
